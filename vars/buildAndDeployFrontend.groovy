...
def call(Map config = [:]) {
    ...
    // Named stage() calls here (rather than one big block) so Jenkins'
    // Pipeline Graph View / Stage View renders real per-phase timing,
    // same as any other multi-stage pipeline.
    stage('Checkout frontend') {
        dir('frontend-src') {
            ...
            commitInfo = "quickmart-frontend @ ${shortSha} ${author}\n${message}"
    ...
    stage('Build & deploy to S3') {
        dir('frontend-src') {
            ...
            // package.json + package-lock.json - same pattern used by
            // BARC-UI's pipeline (tran-ec2-deployment-config). Skips npm
            // entirely when dependencies haven't changed since the last
            // build, which is most builds. Copied in/out rather than
            // symlinked: npm's own atomic renames (@npmcli/*) don't
            // reliably handle node_modules itself being a symlink and fail
            // partway through install with ENOTEMPTY.
    ...
    stage('SonarQube analysis') {
        dir('frontend-src') {
            // Native sonar-scanner (installed in the Jenkins container, see
            // pinakaone-iac user_data.sh.tpl). Config in the app repo's
            // sonar-project.properties. Token fetched from Secrets Manager
            // at runtime (not stored in Jenkins).
            def sonarToken = sh(
            ...
            if (sonarToken) {
                ...
                currentBuild.description = "<a href=\"${sonarUrl}\" target=\"_blank\">SonarQube report</a><br/>${commitInfo}"
            } else {
    ...
    stage('Invalidate CloudFront') {
        // Cross-account credentials (CloudFront is in the paid account,
        // this instance's own role is sandbox-account only) - fetched
        // fresh each run rather than stored as a Jenkins credential, so
        // rotating the IAM user's keys in Terraform needs no matching
        // Jenkins-side change.
        //
        // `set +x` is required here: Jenkins' sh step traces every command
        // to the build log by default (the "+ ..." lines), which would
        // otherwise print the secret and both derived keys in plaintext to
        // anyone who can read the build console.
        sh """
            set +x
