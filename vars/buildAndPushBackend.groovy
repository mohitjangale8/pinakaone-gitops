...
def call(Map config = [:]) {
    ...
    stage('Checkout backend') {
        dir('backend-src') {
            ...
            commitInfo = "quickmart-backend @ ${shortSha} ${author}\n${message}"
    ...
    stage('SonarQube analysis') {
        dir('backend-src') {
            ...
            // see pinakaone-iac user_data.sh.tpl). Token fetched from
            // Secrets Manager at runtime (not stored in Jenkins). Uses the
            // sonar.* properties declared in the app's pom.xml.
            // Link goes FIRST in the description - the build history list
            // truncates long descriptions, so anything after a couple
            // lines (like the commit message) can get clipped out of view.
            def sonarToken = sh(
    ...
    stage('Update gitops config') {
        // Workspace root here is this same gitops repo, already checked out
        // by the pipeline's own "checkout scm" stage.
